                ...
                <div class="row mt-1">
                    <div class="col-sm-12 col-lg-12">
                        <div class="card">
                            ...
                            <form class="form-horizontal" action="{{ route('delay.store') }}" method="POST">
                                @csrf
                                <div class="card-body">
                                    {{-- row 2 --}}
                                    <div class="row mb-3">
                    ...
                    <div class="col-sm-12 col-md-12  col-lg-12 col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">ساعات الاتاخير
                                </h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <table id="example1"
                                                ...
                                                <tbody>
                                                    @foreach ($delaies as $key => $delay)
                                                        <tr class="odd">
                                                            <td>{{ $loop->iteration }}</td>
                                                            <td class="dtr-control sorting_1" tabindex="0">
                                                                {{ $delay->staffs->name }}</td>
                                                            <td>{{ $delay->date }}</td>
                                                            <td>{{ $delay->hours }}</td>
                                                            <td>{{ $delay->mints }}</td>
                                                            <td>{{ $delay->notes }}</td>
                                                            <td class="d-flex justify-content-center">
                                                                <button type="submit" class="btn bg-secondary">
                                                                    <a href="{{ route('delay.edit', $delay->id) }}"
                                                                        class="text-white"><i class="far btn fa-edit "
                                                                            aria-hidden="true"></i></a>
                                                                </button>
                                                                <form method="POST"
                                                                    action="{{ route('delay.destroy', $delay->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
