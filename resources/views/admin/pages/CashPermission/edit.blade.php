            ...
            <div class="row mt-1">
                <div class="col-sm-12 col-lg-12">
                    <div class="card">
                        ...
                        <form class="form-horizontal" action="{{ route('cashPermission.update' , $totals->id) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                ...
                                <table id="example2" class="table table-bordered table-hover">
                                    ...
                                    <tbody id="tbody">
                                        @foreach($cashPermissions as $key => $cashPermission)
                                        <tr>
                                            <td style=" width: 25%;">
                                                <select name="data[items_id][]" class="form-control drop">
                                                    <option value="">اختر الصنف</option>
                                                    @foreach($items as $item)
                                                    <option value="{{ $item->id }}" @if($cashPermission->items_id ==
                                            ...
                                            <td>
                                                <input type="number" class="form-control"
                                                    value="{{$cashPermission->quantity}}" id="qu" placeholder="الكمية"
                                                    name="data[quantity][]">
                                            </td>
                                            <td>
                                                <input type="number" class="form-control"
                                                    value="{{$cashPermission->price}}" placeholder="السعر"
                                                    name="data[price][]">
                                            </td>
                                            <td>
