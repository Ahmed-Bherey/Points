                ...
                <div class="row mt-1">
                    <div class="col-sm-12 col-lg-12">
                        <div class="card">
                            ...
                            <form class="form-horizontal" action="{{ route('filterData.store') }}" method="POST">
                                @csrf
                                <div class="card-body">
                                    {{-- row 2 --}}
                                    <div class="row mb-3">
                    ...
                    <div class="col-sm-12 col-md-12  col-lg-12 col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title" style="float:right; font-weight:bold;"> تسجيل بيانات صيانة لعميل
                                </h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                                    ...
                                                    @foreach ($filters as $key => $filter)
                                                        <tr class="odd">
                                                            ...
                                                            <td class="dtr-control sorting_1" tabindex="0">
                                                                {{ $filter->clients->name }}</td>
                                                            <td>{{ $filter->status }}</td>
                                                            <td>{{ $filter->device_name }}</td>
                                                            <td class="d-flex justify-content-center">
                                                                <button type="submit" class="btn btn bg-secondary">
                                                                    <a href="{{ route('filterData.edit', $filter->id) }}"
                                                                        class="text-white"><i class="far btn fa-edit "
                                                                            aria-hidden="true"></i></a>
                                                                </button>
                                                                <form method="POST"
                                                                    action="{{ route('filterData.destroy', $filter->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
