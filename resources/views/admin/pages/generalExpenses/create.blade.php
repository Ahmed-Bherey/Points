            ...
            <div class="row mt-1">
                <div class="col-sm-12 col-lg-12">
                    <div class="card card-info">
                        ...
                        <form action="{{route('generalExpense.store')}}" method="POST">
                            @csrf
                            <div class="card-body">
                                {{-- row 1 --}}
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-4 form-floating">
                                        <select required class="form-control" name="publicAccount_id" id="expense">
                                            ...
                                            <option value="{{ $generalAccount->id }}">{{$generalAccount->name}}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row mb-3">
                                    <div class="col-sm-4 form-floating">
                                        <input type="number" class="form-control" id="" placeholder="استهلاك"
                                            name="consumption">
                ...
                <div class="col-sm-12 col-md-12  col-lg-12 col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title" style="float:right; font-weight:bold;"> المصروفات العامة
                            </h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="example1"
                                            ...
                                            <tbody>
                                                @foreach($generalExpenses as $key => $generalExpense)
                                                <tr class="odd">
                                                    <td>{{$loop->iteration}}</td>
                                                    <td class="dtr-control sorting_1" tabindex="0">
                                                        {{$generalExpense->date}}</td>
                                                    <td></td>
                                                    <td>{{$generalExpense->amount}}</td>
                                                    <td>{{$generalExpense->consumption}}</td>
                                                    <td>{{$generalExpense->treasuries->name}}</td>
                                                    <td>{{$generalExpense->banks->name}}</td>
                                                    <td>{{$generalExpense->notes}}</td>
                                                    <td>
                                                        <a href="{{ route('generalExpense.edit', $generalExpense->id) }}"
                                                            class="btn bg-secondary"><i class="far fa-edit"
                                                                aria-hidden="true"></i></a>
                                                        <a href="{{ route('generalExpense.destroy', $generalExpense->id) }}"
